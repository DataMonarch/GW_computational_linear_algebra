...
public class GeomTransExample4 {
	...
	double[][] E = MatrixTool.matrixMult (C, A);    // Apply CA
	...
	// Apply matrix E to each point (treated as a vector)
	ArrayList<LineSegment> rect3 = new ArrayList<LineSegment>();
	for (LineSegment L: rect) {
	    LineSegment L3 = L.clone ();
	    L3.start = MatrixTool.matrixVectorMult (E, L.start);   // Apply to endpoint.
	    L3.end = MatrixTool.matrixVectorMult (E, L.end);       // And to other.
	    rect3.add (L3);
	}
